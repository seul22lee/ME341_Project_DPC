...
def DPC_loss(model_output: torch.Tensor, target: torch.Tensor, u_output: torch.Tensor, c_fut: torch.Tensor, return_all=False):
    ...
    low_violation = F.relu(c_fut[:, :, 0] - model_output_x2)
    up_violation = F.relu(model_output_x2 - c_fut[:, :, 1])

    low_violation = low_violation + 0.5 * (low_violation > 0).float()
    up_violation = up_violation + 0.5 * (up_violation > 0).float()

    constraint_loss = (low_violation ** 2) + (up_violation ** 2)

    tracking_loss = torch.sqrt(errors.mean())
    smoothness_loss = 10 * torch.sqrt(squared_diff.mean())
    constraint_loss = torch.sqrt(constraint_loss.mean())
...
def DPC_loss_notused(model_output: torch.Tensor, target: torch.Tensor, u_output: torch.Tensor, c_fut: torch.Tensor, return_all=False):
    model_output = model_output.median(dim=-1).values
    model_output_x1 = model_output[:, :, 0]
    target_x1 = target[:, :, 0]
    errors = (target_x1 - model_output_x1) ** 2
    ...
    model_output_x2 = model_output[:, :, 1]
    low_violation_mask = model_output_x2 < c_fut[:, :, 0]
    up_violation_mask = model_output_x2 > c_fut[:, :, 1]
    penalty = 5.0
    low_violation = penalty * low_violation_mask.float()
    up_violation = penalty * up_violation_mask.float()
